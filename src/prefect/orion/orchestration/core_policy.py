...
from app.orion.policy import GreenTaskPolicy, GreenFlowPolicy
...
class SecureTaskConcurrencySlots(BaseOrchestrationRule):
    ...
    async def before_transition(
        ...
        filtered_limits = await concurrency_limits.filter_concurrency_limits_for_orchestration(
...
class HandlePausingFlows(BaseOrchestrationRule):
    ...
    async def before_transition(
        ...
        if not initial_state.is_running():
            await self.reject_transition(
                state=None,
                reason="Cannot pause flows that are not currently running.",
            )
            return
...
class HandleTaskTerminalStateTransitions(BaseOrchestrationRule):
    ...
    async def before_transition(
        ...
        if proposed_state.is_running() and (
            ...
            if flow_retrying:
                context.run.run_count = (
                    0  # reset run count to preserve retry behavior
                )
                await self.rename_state("Retrying")
                return
...
class HandleFlowTerminalStateTransitions(BaseOrchestrationRule):
    ...
    async def before_transition(
        ...
        if (
            proposed_state.is_scheduled()
            and proposed_state.name == "AwaitingRetry"
        ):
            ...
            if api_version is None or api_version >= Version("0.8.4"):
                ...
                updated_policy["resuming"] = False
                updated_policy["pause_keys"] = set()
                context.run.empirical_policy = core.FlowRunPolicy(
                    **updated_policy
                )
            ...
            await self.abort_transition(
                reason="This run has already terminated."
            )
